...
class ResidualAdaptiveSwinFusion(nn.Module):
    """Add a small input-adaptive Swin correction to a CNN feature map."""
    ...
    def __init__(
        ...
        gate_hidden = max(normalized_in_channels // normalized_gate_reduction, 16)
        ...
        self.channel_gate = nn.Sequential(
            nn.Conv2d(normalized_in_channels * 2, gate_hidden, kernel_size=1, bias=True),
            nn.SiLU(inplace=True),
            nn.Conv2d(gate_hidden, normalized_in_channels, kernel_size=1, bias=True),
        )
        self.spatial_gate = nn.Sequential(
            nn.Conv2d(3, spatial_hidden, kernel_size=3, padding=1, bias=True),
            nn.SiLU(inplace=True),
    ...
    @staticmethod
    def _detail_map(x: Tensor) -> Tensor:
        ...
        return detail / (detail_scale + 1e-6) - 1.0
    ...
    def detail_mean(self) -> float:
        return self._last_detail_mean

    @property
    def detail_max(self) -> float:
        return self._last_detail_max
    ...
    def forward(self, x: Tensor) -> Tensor:
        swin = self.swin(x)
        diff = swin - x
        delta = self.delta_proj(torch.cat((x, swin, diff), dim=1))

        channel_context = torch.cat(
            (
                F.adaptive_avg_pool2d(x, 1),
                F.adaptive_avg_pool2d(swin, 1),
            ),
            dim=1,
        )
        channel_gate = self.channel_gate(channel_context)

        detail = self._detail_map(x)
        spatial_context = torch.cat(
            (
                x.mean(dim=1, keepdim=True),
                swin.mean(dim=1, keepdim=True),
                detail,
            ),
            dim=1,
        )
