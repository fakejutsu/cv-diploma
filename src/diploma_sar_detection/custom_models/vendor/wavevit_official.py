...
try:
    ...
    class DWT_2D(nn.Module):
        def __init__(self, wave='haar'):
            super().__init__()
            self.register_buffer('w_ll', torch.tensor([[[[0.5, 0.5], [0.5, 0.5]]]], dtype=torch.float32))
            self.register_buffer('w_lh', torch.tensor([[[[-0.5, -0.5], [0.5, 0.5]]]], dtype=torch.float32))
            self.register_buffer('w_hl', torch.tensor([[[[-0.5, 0.5], [-0.5, 0.5]]]], dtype=torch.float32))
            self.register_buffer('w_hh', torch.tensor([[[[0.5, -0.5], [-0.5, 0.5]]]], dtype=torch.float32))

        def forward(self, x):
            channels = int(x.shape[1])
            filters = torch.cat((self.w_ll, self.w_lh, self.w_hl, self.w_hh), dim=0).to(dtype=x.dtype)
            weight = filters.repeat(channels, 1, 1, 1)
            return F.conv2d(x, weight, stride=2, groups=channels)
    ...
    class IDWT_2D(nn.Module):
        ...
        def forward(self, x):
            ...
            return F.conv_transpose2d(x, weight, stride=2, groups=channels)
